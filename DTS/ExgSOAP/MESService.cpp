...
static MESServiceSoapProxy proxy(SOAP_C_UTFSTRING);
...
{
    qDebug() << "MESServiceSoapService::WorkOrder" << tempuri__WorkOrder->WorkOrderXml;

    //MESServiceSoapProxy proxy(SOAP_C_UTFSTRING);

    int res = proxy.WorkOrder(tempuri__WorkOrder, tempuri__WorkOrderResponse);

    qDebug() << res << tempuri__WorkOrder->WorkOrderXml << tempuri__WorkOrderResponse.WorkOrderResult;
    do
    {
        if (res == SOAP_OK)
        {
            ...
            QSqlQuery dbQuery(QSqlDatabase::database(QSQL_DB_STRING_LOCAL));
            QString dbSQL = "UPDATE MES_WorkOrder SET State=? WHERE UID=?";
            QVariantList dbParam;
            dbParam << uid.text() << state.text();
...
int MESServiceSoapService::Login(_tempuri__Login *tempuri__Login, _tempuri__LoginResponse &tempuri__LoginResponse)
{
    ...
    return proxy.Login("http://10.1.1.13:8001/", "", tempuri__Login, tempuri__LoginResponse);
}
